...
public class Exchanger<V> {
    ...
    static final class Node {
        ...
        int bound;              // 交换器的最后记录值
        int collides;           // 记录的 CAS 失败数 Number of CAS failures at current bound
        int hash;               // 伪随机的自旋数
        Object item;            // 这个线程的数据项  => 先执行的线程
        volatile Object match;  // 另一个线程（释放线程）的数据项  => 后执行的线程
...
/**
 *没有多线程并发操作 Exchange 的时候，使用 slotExchange 就足够了。 slot 是一个 node 对象。
 *  当出现并发了，一个 slot 就不够了，就需要使用一个 node 数组 arena 操作了。
 */
        Object v;
        Object item = (x == null) ? NULL_ITEM : x; // translate null args
        // arena是null就执行后面的slotExchange方法
